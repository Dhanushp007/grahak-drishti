  ...
  return (
    <main className="dashboard-shell">
      ...
      <section className="dashboard-content issue-drilldown" id="summary">
        ...
        <div className="drill-grid"><section className="panel"><div className="panel-heading"><div><p className="overline">India distribution</p><h2>Where reports concentrate</h2></div><span className="period">{issue.states_affected} states</span></div><div className="state-list">{(issue.geography || []).slice(0, 8).map((point) => <div className="state-list-row" key={point.state}><span>{point.state}</span><strong>{point.reports}</strong><small>{point.evidence_backed} evidence-backed</small></div>)}</div></section><section className="panel routing-panel"><div className="panel-heading"><div><p className="overline">Recommended next step</p><h2>Advisory routing</h2></div><ArrowUpRight size={18} /></div><strong className="route-name">{String(route.route || "review").replaceAll("_", " ")}</strong><p>{route.reason || "An authorized analyst should review this aggregate signal."}</p><small>Confidence: {Math.round(Number(route.confidence || 0) * 100)}% · {route.source || "Demo rule"}</small>{issue.potential_dark_pattern_count > 0 && <div className="pattern-note"><ShieldAlert size={16} /><span><strong>Potential dark pattern detected</strong> in {issue.potential_dark_pattern_count} aggregate reports. Review the pattern as an advisory concern.</span></div>}<div className="advisory-note"><ShieldAlert size={16} /> Recommendations are advisory and do not represent a live government handoff.</div></section></div>
